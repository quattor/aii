...
sub _link_filepath
{
    ...
    if ( defined($pxe_config->{$cmd}) ) {
        ...
        my $fqdn = $self->_get_fqdn($cfg);
        $self->debug(2, "No $cmd defined for $fqdn");
    }
    return undef;
}
...
sub _write_pxelinux_config
{
    my ($self, $cfg) = @_;
    my $pxe_config = $cfg->getElement (PXEROOT)->getTree;
    my $fh = CAF::FileWriter->open ($self->_filepath ($cfg, PXE_VARIANT_PXELINUX),
                    log => $self, mode => 0644);
...
sub _pxelink
{
    ...
    if (!$cmd) {
        ...
        $self->debug (5, "Configuring on $path");
    } elsif ($cmd eq BOOT) {
        $path = $this_app->option (LOCALBOOT);
        unless ($path =~ m{^([-.\w]+)$}) {
        ...
        $path = $self->_link_filepath($cfg, $cmd, $variant);
        if (! $self->file_exists($path) ) {
            my $fqdn = $self->_get_fqdn($cfg);
            $self->error("Missing $cmd config file for $fqdn: $path");
            return 0;
        }
        $self->debug (5, "Using $cmd from: $path");
    } elsif ($cmd eq INSTALL) {
        $path = $self->_filepath ($cfg, $variant);
        $self->debug (5, "Installing on $path");
    } else {
        $self->debug (5, "Unknown command");
        return 0;
    }
...
sub Status
{
    ...
    foreach my $variant_constant (@PXE_VARIANTS) {
        ...
        my $fqdn = $self->_get_fqdn($cfg);
        my $rescue = $self->_link_filepath($cfg, RESCUE, $variant);
        my $firmware = $self->_link_filepath($cfg, FIRMWARE, $variant);
        my $livecd = $self->_link_filepath($cfg, LIVECD, $variant);
